...
class ProducerProvider extends ChangeNotifier {
  ...
  Future<void> connect(String bootstrapServers) async {
    try {
      developer.log('Attempting to connect producer to Kafka at $bootstrapServers via FFI');
      _producer = KafkaFFI.createProducer(bootstrapServers);
      _bootstrapServers = bootstrapServers;
      _isConnected = true;
      developer.log('Successfully connected producer to Kafka at $bootstrapServers');
      notifyListeners();
    } catch (e, stackTrace) {
      developer.log('Failed to connect producer to Kafka: $e', stackTrace: stackTrace);
      _isConnected = false;
  ...
  Future<void> disconnect() async {
    try {
      ...
      developer.log('Failed to disconnect producer: $e', stackTrace: stackTrace);
      try {
        if (_producer != null) {
          KafkaFFI.closeClient(_producer!);
          _producer = null;
